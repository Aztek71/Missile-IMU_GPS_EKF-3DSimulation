...
def lancer_simulation_3d():
    # ---------
    # TIRAGE ALÉATOIRE DE LA GRAINE (change à chaque exécution)
    # ---------
    graine = np.random.randint(0, 1_000_000)
    rng = np.random.default_rng(graine)
    p = 0.7
    print(f"Scénario généré avec la graine aléatoire : {graine}")

    # ---------
    # PARAMÈTRES DE CONFIGURATION (activation aléatoire des événements)
    # ---------
    perte_signal_active = bool(rng.choice([True, False]))
    manoeuvre_esquive_active = bool(rng.choice([True, False]))
    manoeuvre_zigzag_active = bool(rng.choice([True, False]) < p)
    print(f"Brouillage: {perte_signal_active} | Esquive: {manoeuvre_esquive_active} | Zigzag: {manoeuvre_zigzag_active}")
    ...
    # ---------
    # TIRAGE ALÉATOIRE DU RESTE DU SCÉNARIO
    # ---------
    # Position initiale de la cible (distance et géométrie variables)
    x_t = rng.uniform(20.0, 60.0)
    y_t = rng.uniform(80.0, 160.0)
    z_t = rng.uniform(40.0, 120.0)

    # Vitesse initiale de la cible (cap et vitesse horizontale/verticale variables)
    vitesse_cible = rng.uniform(1.0, 3.0)
    cap_cible = rng.uniform(0, 2 * np.pi)
    vx_t = vitesse_cible * np.cos(cap_cible)
    vy_t = vitesse_cible * np.sin(cap_cible)
    vz_t = rng.uniform(-0.5, 0.5)

    # Paramètres du missile / interception (vitesse et poussée initiales variables)
    v_s = rng.uniform(12.0, 20.0)
    a_prop = rng.uniform(8.0, 16.0)

    # Paramètres des manœuvres (timing et amplitude variables)
    k_esquive = rng.integers(80, 250)
    amplitude_esquive_horiz = rng.uniform(1.2, 1.8)
    amplitude_esquive_vert = rng.uniform(1.0, 1.5)

    periode_zigzag = rng.integers(40, 90)
    phase_zigzag = rng.integers(10, 30)
    angle_zigzag_deg = rng.uniform(20.0, 45.0)
    sens_zigzag_initial = rng.choice([-1, 1]) * 2
    ...
    # ---------
    X = np.array([0.0, 0.0, 0.0, psi_initial, gamma_initial, v_s,
    ...
    sens_zigzag = sens_zigzag_initial
    ...
    for k in range(temps):
        ...
        if manoeuvre_esquive_active and k == k_esquive:
            vx_t_old, vz_t_old = vx_t, vz_t
            vx_t = -vy_t * amplitude_esquive_horiz
            vy_t = vx_t_old * amplitude_esquive_horiz
            vz_t = -vz_t_old * amplitude_esquive_vert - 0.4  # rupture en piqué/ressource

        # 3. ZIGZAG PÉRIODIQUE (dans le plan horizontal)
        if manoeuvre_zigzag_active and k > phase_zigzag and ((k - phase_zigzag) % periode_zigzag == 0):
            angle_virage = np.radians(angle_zigzag_deg) * sens_zigzag
        ...
        if k > 20 and dist_phys < 2.5:
            ...
            print("Simulation terminée sans interception.")
            break
    ...
    ax1.scatter(*history_true_suiveur[-1], color='green', s=50, label="Impact")
    ax1.set_xlabel("X (m)")
    ax1.set_ylabel("Y (m)")
